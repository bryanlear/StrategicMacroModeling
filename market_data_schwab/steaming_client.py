...
from collections import deque, OrderedDict #for order book display and trade log
...
MAX_BOOK_LEVELS_DISPLAY = 10 # Number bid/ask levels
MAX_TRADES_DISPLAY = 10 # Number of recent trades 
...
async def handle_level_two_book_data(data, book_type="NASDAQ_BOOK"): # NASDAQ_BOOK or NYSE_BOOK
    ...
    try:
        ...
        # Create new OrderedDicts for update
        ...
        # Update main order book for symbol
...
async def display_bookmap_like_textual(symbol=ETF_SYMBOL):
    ...
    trades = list(trade_log.get(symbol, deque())) 
    ...
    logger.info("\n--- BIDS (Price: Volume) ---")
    bid_levels_shown = 0
    for price, volume in list(book['bids'].items())[:MAX_BOOK_LEVELS_DISPLAY]: 
...
async def main_streaming_loop():
    ...
    try:
        ...
        # Choose ONE book handlers based on ETF's listing exchange:
        ...
        # IMPORTANT: Choose correct book for ticker 
        # For SPY (ARCA part of NYSE group): NYSE_BOOK
