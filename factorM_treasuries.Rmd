...
barplot(yield_changes_mean, main = "US Treasury Yield Changes", ylab = "Daily Mean")
barplot(yield_changes_vol, main = "US Treasury Yield Changes", ylab = "Daily Volatilities")

library(dplyr)
selected_treasuries <- yield_changes_clean%>% select("diff_DGS3MO", "diff_DGS2", "diff_DGS10")
pairs(selected_treasuries, main = "Treasury Scatterplot Matrix")
knitr::opts_chunk$set(echo = TRUE)
```
...
sp500_data <- fredr_series_observations(
  series_id = "SP500",
...
sp500_value_data <- data.frame(sp500_data$value)
sp500_pca <- PCA(sp500_value_data, graph = TRUE,scale.unit = TRUE)
...
```{r PCA, include = TRUE}
library(missMDA)
treasury_data_values <- treasury_transposed[,-1]
#dataframed_treasury <- data.frame(treasury_data_values)
imputed_values <- imputePCA(treasury_data_values)
pca_meta_scaled <- PCA(imputed_values, graph = TRUE,scale.unit = TRUE)

pca_meta_scaled$var$coord[,1:2]
