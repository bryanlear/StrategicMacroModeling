...
OUTPUT_DIR = "data"
INVESTMENT_CSV_FILE = os.path.join(OUTPUT_DIR, 'us_investment_net_exports_indicators.csv')
INVESTMENT_PARQUET_FILE = os.path.join(OUTPUT_DIR, 'us_investment_net_exports_indicators.parquet')
...
if not FRED_API_KEY:
    ...
    print("Fred API client initialized for Investment & Net Exports.")

END_DATE = datetime.datetime.now()
START_DATE = END_DATE - datetime.timedelta(days=20 * 365) # 20 years for investment/FDI data

series_map_investment = {
    "Gross Private Domestic Investment": "GPDI",
    "Real Gross Private Domestic Investment": "GPDIC1",
    "FDI in U.S. Transactions": "ROWFDIQ027S",
    "FDI in U.S. Level (Current Cost)": "ROWFDNQ027S",
    "FDI in U.S. Equity Level (Market Value)": "BOGZ1FL263092141Q",
    "FDI in U.S. Real Estate Level (Market Value)": "BOGZ1FL265014003Q",
...
def fetch_fred_data_via_api(fred_client, series_dict, start_date, end_date):
    ...
    for short_name, series_id in series_dict.items():
        try:
            ...
            empty_idx = pd.date_range(start=start_str, end=end_str, freq='QS') if not df_list else df_list[0].index # Quarterly start for some investment
...
if __name__ == "__main__":
    if fred is None:
        ...
        print("Fetching US Investment & Net Exports Indicators from FRED using fredapi...")
        us_investment_df = fetch_fred_data_via_api(fred, series_map_investment, START_DATE, END_DATE)

        if not us_investment_df.empty:
            print("\n--- Combined US Investment & Net Exports Indicators Data (Last 5 entries) ---")
            print(us_investment_df.tail())
            print("\n--- Combined US Investment & Net Exports Indicators Data (First 5 entries) ---")
            print(us_investment_df.head())
            print(f"\nShape of the DataFrame: {us_investment_df.shape}")
            
            store_data(us_investment_df, INVESTMENT_CSV_FILE, INVESTMENT_PARQUET_FILE)
        else:
            print("Failed to retrieve any investment & net exports data from FRED via API. Nothing to store.")
